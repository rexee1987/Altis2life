...
*/
	...
	if (__GETC__(life_coplevel) > 1)  then {
		player setObjectTextureGlobal [0, "texture\police_shirt.paa"]; // this is for all cops whitelisted or public.
	};
	...
	if ((uniform player) == "U_B_CombatUniform_mcam")  then {
		player setObjectTextureGlobal [0, "texture\swat_shirt.paa"];
	};
	if ((uniform player) == "H_HelmetB_light_black")  then {
		player setObjectTextureGlobal [0, "texture\swat_helmet.paa"];
	};
	if ((uniform player) == "H_Cap_blk")  then {
		player setObjectTextureGlobal [0, "texture\swat_s_cap.paa"];
	};
	if ((uniform player) == "V_TacVest_blk")  then {
		player setObjectTextureGlobal [0, "texture\police_vest.paa"];
	};
