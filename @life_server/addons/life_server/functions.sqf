...
compileFinal "
	...
	if(_val > 9999999) exitWith {hint ""You can't deposit more then $9,999,999"";};
	if(_val < 0) exitwith {};
	...
	hint format[""Du hast $%1 in deiner Bank"",[_val] call life_fnc_numberText];
...
compileFinal "
	private[""_val""];
	_val = parseNumber(ctrlText 2702);
	if(_val > 9999999) exitWith {hint ""You can't withdraw more then $9,999,999"";};
	if(_val < 0) exitwith {};
	if(!([str(_val)] call fnc_isnumber)) exitWith {hint ""That isn't in an actual number format.""};
